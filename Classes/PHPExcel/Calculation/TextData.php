...
class PHPExcel_Calculation_TextData {
	...
	private static ?array $_invalidChars = Null;
	 ...
	 * @return	string
	 */
	public static function TRIMSPACES(mixed $stringValue = '') {
		$stringValue	= PHPExcel_Calculation_Functions::flattenSingleValue($stringValue);
	...
	public static function ASCIICODE($characters) {
		...
		if ((function_exists('mb_strlen')) && (function_exists('mb_substr'))) {
			if (mb_strlen((string) $characters, 'UTF-8') > 1) { $character = mb_substr((string) $characters, 0, 1, 'UTF-8'); }
			return self::_uniord($character);
		} else {
			if (strlen((string) $characters) > 0) { $character = substr((string) $characters, 0, 1); }
		...
		if (!is_bool($needle)) {
			...
			if (($offset > 0) && (PHPExcel_Shared_String::CountCharacters($haystack) > $offset)) {
				...
				if (function_exists('mb_strpos')) {
					$pos = mb_strpos((string) $haystack, (string) $needle, --$offset, 'UTF-8');
				} else {
		...
		if (!is_bool($needle)) {
			...
			if (($offset > 0) && (PHPExcel_Shared_String::CountCharacters($haystack) > $offset)) {
				...
				if (function_exists('mb_stripos')) {
					$pos = mb_stripos((string) $haystack, (string) $needle, --$offset,'UTF-8');
				} else {
					$pos = stripos((string) $haystack, (string) $needle, --$offset);
				}
				if ($pos !== false) {
					return ++$pos;
				}
	...
	public static function LEFT($value = '', $chars = 1) {
		...
		if (function_exists('mb_substr')) {
			...
			return substr((string) $value, 0, $chars);
		}
	}	//	function LEFT()
	...
	public static function MID($value = '', $start = 1, $chars = null) {
		...
		if (function_exists('mb_substr')) {
			...
			return substr((string) $value, --$start, $chars);
		}
	}	//	function MID()
	...
	public static function RIGHT($value = '', $chars = 1) {
		...
		if ((function_exists('mb_substr')) && (function_exists('mb_strlen'))) {
			...
			return substr((string) $value, strlen((string) $value) - $chars);
		}
	}	//	function RIGHT()
	...
	public static function SUBSTITUTE($text = '', $fromText = '', $toText = '', $instance = 0) {
		...
		if ($instance == 0) {
			if(function_exists('mb_str_replace')) {
				...
				return str_replace($fromText,$toText,(string) $text);
			}
		} else {
			$pos = -1;
			while($instance > 0) {
				if (function_exists('mb_strpos')) {
					$pos = mb_strpos((string) $text, (string) $fromText, $pos+1, 'UTF-8');
				} else {
					$pos = strpos((string) $text, (string) $fromText, $pos+1);
				}
				if ($pos === false) {
					break;
				}
	 ...
	 * @return	boolean
	 */
	public static function RETURNSTRING(mixed $testValue = '') {
