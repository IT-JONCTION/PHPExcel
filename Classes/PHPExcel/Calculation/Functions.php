...
class PHPExcel_Calculation_Functions {
	...
	public static function _ifCondition($condition) {
		...
		if (!in_array($condition[0],['>', '<', '='])) {
			if (!is_numeric($condition)) { $condition = PHPExcel_Calculation::_wrapResult(strtoupper((string) $condition)); }
			return '='.$condition;
		} else {
...
if (!function_exists('money_format')) {
	function money_format($format, $number) {
		...
		if (!preg_match($regex, (string) $format, $fmatch)) {
			trigger_error("No format specified or invalid format", E_USER_WARNING);
			return $number;
		}
		$flags = ['fillchar'	=> preg_match('/\=(.)/', $fmatch[1], $match) ? $match[1] : ' ', 'nogroup'	=> preg_match('/\^/', $fmatch[1]) > 0, 'usesignal'	=> preg_match('/\+|\(/', $fmatch[1], $match) ? $match[0] : '+', 'nosimbol'	=> preg_match('/\!/', $fmatch[1]) > 0, 'isleft'	=> preg_match('/\-/', $fmatch[1]) > 0];
		...
		$n = strlen((string) $prefix) + strlen($currency);
		if ($left > 0 && $left > $n) {
			if ($flags['isleft']) {
		...
		$format = str_replace($fmatch[0], $number, (string) $format);
		return $format;
