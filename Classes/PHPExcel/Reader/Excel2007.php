...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
  ...
  */
 private static ?\PHPExcel_Reader_Excel2007_Theme $_theme = NULL;
	...
	public function canRead($pFilename)
	{
		...
		if ($zip->open($pFilename) === true) {
			...
			$rels = simplexml_load_string((string) $this->_getFromZipArchive($zip, "_rels/.rels"));
			if ($rels !== false) {
	...
	public function listWorksheetNames($pFilename)
	{
		...
		$rels = simplexml_load_string(
		    (string) $this->_getFromZipArchive($zip, "_rels/.rels")
		); //~ http://schemas.openxmlformats.org/package/2006/relationships");
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					$xmlWorkbook = simplexml_load_string(
					    (string) $this->_getFromZipArchive($zip, "{$rel['Target']}")
	...
	public function listWorksheetInfo($pFilename)
	{
		...
		$rels = simplexml_load_string((string) $this->_getFromZipArchive($zip, "_rels/.rels")); //~ http://schemas.openxmlformats.org/package/2006/relationships");
		foreach ($rels->Relationship as $rel) {
			if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
				$dir = dirname($rel["Target"]);
				$relsWorkbook = simplexml_load_string((string) $this->_getFromZipArchive($zip, "$dir/_rels/" . basename($rel["Target"]) . ".rels"));  //~ http://schemas.openxmlformats.org/package/2006/relationships");
				...
				$xmlWorkbook = simplexml_load_string((string) $this->_getFromZipArchive($zip, "{$rel['Target']}"));  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
				if ($xmlWorkbook->sheets) {
					$dir = dirname($rel["Target"]);
					foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
						$tmpInfo = ['worksheetName' => (string) $eleSheet["name"], 'lastColumnLetter' => 'A', 'lastColumnIndex' => 0, 'totalRows' => 0, 'totalColumns' => 0];
...
//				echo 'Adjusted Formula is '.$value.'<br />';
	...
	public function _getFromZipArchive(ZipArchive $archive, $fileName = '')
	{
		...
		{
			$fileName = substr((string) $fileName, strpos((string) $fileName, '//') + 1);
		}
	...
	public function load($pFilename)
	{
		...
		$wbRels = simplexml_load_string((string) $this->_getFromZipArchive($zip, "xl/_rels/workbook.xml.rels")); //~ http://schemas.openxmlformats.org/package/2006/relationships");
		foreach ($wbRels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
					...
					$xmlTheme = simplexml_load_string((string) $this->_getFromZipArchive($zip, "xl/{$rel['Target']}"));
		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				case "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties":
					$xmlCore = simplexml_load_string((string) $this->_getFromZipArchive($zip, "{$rel['Target']}"));
					if (is_object($xmlCore)) {
						...
						$docProps->setCreated(strtotime((string) self::array_item($xmlCore->xpath("dcterms:created")))); //! respect xsi:type
						$docProps->setModified(strtotime((string) self::array_item($xmlCore->xpath("dcterms:modified")))); //! respect xsi:type
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					$xmlStrings = simplexml_load_string((string) $this->_getFromZipArchive($zip, "$dir/$xpath[Target]"));  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
					...
					$xmlStyles = simplexml_load_string((string) $this->_getFromZipArchive($zip, "$dir/$xpath[Target]")); //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
					$numFmts = null;
					...
					if (!$this->_readDataOnly && $xmlStyles) {
						foreach ($xmlStyles->cellXfs->xf as $xf) {
							...
							$style = (object) ["numFmt" => $numFmt, "font" => $xmlStyles->fonts->font[intval($xf["fontId"])], "fill" => $xmlStyles->fills->fill[intval($xf["fillId"])], "border" => $xmlStyles->borders->border[intval($xf["borderId"])], "alignment" => $xf->alignment, "protection" => $xf->protection];
							$styles[] = $style;
						...
						foreach ($xmlStyles->cellStyleXfs->xf as $xf) {
							...
							$cellStyle = (object) ["numFmt" => $numFmt, "font" => $xmlStyles->fonts->font[intval($xf["fontId"])], "fill" => $xmlStyles->fills->fill[intval($xf["fillId"])], "border" => $xmlStyles->borders->border[intval($xf["borderId"])], "alignment" => $xf->alignment, "protection" => $xf->protection];
					...
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
							...
							$fileWorksheet = $worksheets[(string) self::array_item($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];
							$xmlSheet = simplexml_load_string((string) $this->_getFromZipArchive($zip, "$dir/$fileWorksheet"));  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
	...
	//									echo 'Value is '.$value.'<br />';
								...
								foreach ($xmlSheet->autoFilter->filterColumn as $filterColumn) {
									...
									if ($filterColumn->filters) {
										...
										foreach ($filters->dateGroupItem as $dateGroupItem) {
											$column->createRule()->setRule(
												NULL,	//	Operator is undefined, but always treated as EQUAL
												['year' => (string) $dateGroupItem["year"], 'month' => (string) $dateGroupItem["month"], 'day' => (string) $dateGroupItem["day"], 'hour' => (string) $dateGroupItem["hour"], 'minute' => (string) $dateGroupItem["minute"], 'second' => (string) $dateGroupItem["second"]],
							...
							if (!$this->_readDataOnly) {
								...
								if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
									$relsWorksheet = simplexml_load_string((string) $this->_getFromZipArchive($zip,  dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels") ); //~ http://schemas.openxmlformats.org/package/2006/relationships");
							...
							if (!$this->_readDataOnly) {
								...
								foreach ($comments as $relName => $relPath) {
									...
									$commentsFile = simplexml_load_string((string) $this->_getFromZipArchive($zip, $relPath) );
							    ...
							    foreach ($vmlComments as $relName => $relPath) {
									...
									$relPath = PHPExcel_Shared_File::realpath(dirname("$dir/$fileWorksheet") . "/" . $relPath);
									$vmlCommentsFile = simplexml_load_string( (string) $this->_getFromZipArchive($zip, $relPath) );
								...
								if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->_readDataOnly) {
									if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
										...
										if ($vmlRelationship != '') {
											// Fetch linked images
											$relsVML = simplexml_load_string((string) $this->_getFromZipArchive($zip,  dirname((string) $vmlRelationship) . '/_rels/' . basename((string) $vmlRelationship) . '.rels' )); //~ http://schemas.openxmlformats.org/package/2006/relationships");
											$drawings = [];
											...
											// Fetch VML document
											$vmlDrawing = simplexml_load_string((string) $this->_getFromZipArchive($zip, $vmlRelationship));
...
// TODO: Autoshapes from twoCellAnchors!
							if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
								...
								if ($xmlSheet->drawing && !$this->_readDataOnly) {
									foreach ($xmlSheet->drawing as $drawing) {
										...
										$relsDrawing = simplexml_load_string((string) $this->_getFromZipArchive($zip,  dirname((string) $fileDrawing) . "/_rels/" . basename((string) $fileDrawing) . ".rels") ); //~ http://schemas.openxmlformats.org/package/2006/relationships");
										$images = [];
											...
											foreach ($relsDrawing->Relationship as $ele) {
												if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image") {
													...
													if ($this->_includeCharts) {
														$charts[self::dir_add($fileDrawing, $ele["Target"])] = ['id'		=> (string) $ele["Id"], 'sheet'	=> $docSheet->getTitle()];
													}
												}
											}
										}
										$xmlDrawing = simplexml_load_string((string) $this->_getFromZipArchive($zip, $fileDrawing))->children("http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing");
											...
											foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
												if ($twoCellAnchor->pic->blipFill) {
													...
													$chartDetails[$docSheet->getTitle().'!'.$thisChart] =
															['fromCoordinate'	=> $fromCoordinate, 'fromOffsetX'		=> $fromOffsetX, 'fromOffsetY'		=> $fromOffsetY, 'toCoordinate'		=> $toCoordinate, 'toOffsetX'			=> $toOffsetX, 'toOffsetY'			=> $toOffsetY, 'worksheetTitle'	=> $docSheet->getTitle()];
												}
											}
										}
		...
		if (!$this->_readDataOnly) {
			$contentTypes = simplexml_load_string((string) $this->_getFromZipArchive($zip, "[Content_Types].xml"));
			foreach ($contentTypes->Override as $contentType) {
				switch ($contentType["ContentType"]) {
					case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
						if ($this->_includeCharts) {
							$chartEntryRef = ltrim($contentType['PartName'],'/');
							$chartElements = simplexml_load_string((string) $this->_getFromZipArchive($zip, $chartEntryRef));
...
//		}
	...
	private static function dir_add($base, $add) {
		return preg_replace('~[^/]+/\.\./~', '', dirname((string) $base) . "/$add");
	}
