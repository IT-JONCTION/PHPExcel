	...
	public function listWorksheetInfo($pFilename)
	{
		...
		while ($xml->read()) {
			if ($xml->name == 'gnm:Sheet' && $xml->nodeType == XMLReader::ELEMENT) {
				$tmpInfo = ['worksheetName' => '', 'lastColumnLetter' => 'A', 'lastColumnIndex' => 0, 'totalRows' => 0, 'totalColumns' => 0];
...
//		echo '</pre><hr />';
//
		$xml = simplexml_load_string((string) $gFileData);
...
//				echo 'Value is ',$cell,'<br />';
				...
				if ($ExprID > '') {
					if (((string) $cell) > '') {
						...
						$this->_expressions[$ExprID] = ['column'	=> $cellAttributes->Col, 'row'		=> $cellAttributes->Row, 'formula'	=> (string) $cell];
