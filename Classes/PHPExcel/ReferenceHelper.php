...
class PHPExcel_ReferenceHelper
{
	/**	Constants				*/
	/**	Regular Expressions		*/
	final public const REFHELPER_REGEXP_CELLREF		= '((\w*|\'[^!]*\')!)?(?<![:a-z\$])(\$?[a-z]{1,3}\$?\d+)(?=[^:!\d\'])';
	final public const REFHELPER_REGEXP_CELLRANGE	= '((\w*|\'[^!]*\')!)?(\$?[a-z]{1,3}\$?\d+):(\$?[a-z]{1,3}\$?\d+)';
	final public const REFHELPER_REGEXP_ROWRANGE		= '((\w*|\'[^!]*\')!)?(\$?\d+):(\$?\d+)';
	final public const REFHELPER_REGEXP_COLRANGE		= '((\w*|\'[^!]*\')!)?(\$?[a-z]{1,3}):(\$?[a-z]{1,3})';

	/**
  * Instance of this class
  */
 private static ?\PHPExcel_ReferenceHelper $_instance = null;
					...
					foreach($matches as $match) {
						...
						if ($match[3].$match[4] !== $modified3.$modified4) {
							if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
								...
								$row = trim((string) $row,'$') + 10_000_000;
								$cellIndex = $column.$row;
		...
		foreach ($pPhpExcel->getWorksheetIterator() as $sheet) {
			foreach ($sheet->getCellCollection(false) as $cellID) {
				...
				if (($cell !== NULL) && ($cell->getDataType() == PHPExcel_Cell_DataType::TYPE_FORMULA)) {
					...
					if (str_contains((string) $formula, $oldName)) {
						$formula = str_replace("'" . $oldName . "'!", "'" . $newName . "'!", (string) $formula);
