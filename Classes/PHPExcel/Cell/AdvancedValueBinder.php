...
class PHPExcel_Cell_AdvancedValueBinder extends PHPExcel_Cell_DefaultValueBinder implements PHPExcel_Cell_IValueBinder
{
    ...
    public function bindValue(PHPExcel_Cell $cell, $value = null)
    {
        ...
        if ($dataType === PHPExcel_Cell_DataType::TYPE_STRING && !$value instanceof PHPExcel_RichText) {
            ...
            // Check for fraction
            if (preg_match('/^([+-]?) *([0-9]*)\s?\/\s*([0-9]*)$/', (string) $value, $matches)) {
            ...
            if (preg_match('/^\-?[0-9]*\.?[0-9]*\s?\%$/', (string) $value)) {
                // Convert value to number
                $value = (float) str_replace('%', '', (string) $value) / 100;
            ...
            if (preg_match('/^'.preg_quote($currencyCode).' *(\d{1,3}('.preg_quote($thousandsSeparator).'\d{3})*|(\d+))('.preg_quote($decimalSeparator).'\d{2})?$/', (string) $value)) {
                // Convert value to number
                $value = (float) trim(str_replace([$currencyCode, $thousandsSeparator, $decimalSeparator], ['', '', '.'], (string) $value));
                $cell->setValueExplicit( $value, PHPExcel_Cell_DataType::TYPE_NUMERIC);
                // Set style
                $cell->getWorksheet()->getStyle( $cell->getCoordinate() )
                    ->getNumberFormat()->setFormatCode(
                ...
                // Convert value to number
                $value = (float) trim(str_replace(['$', ','], '', (string) $value));
                $cell->setValueExplicit( $value, PHPExcel_Cell_DataType::TYPE_NUMERIC);
                // Set style
                $cell->getWorksheet()->getStyle( $cell->getCoordinate() )
