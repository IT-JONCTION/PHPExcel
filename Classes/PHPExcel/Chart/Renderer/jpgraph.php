...
class PHPExcel_Chart_Renderer_jpgraph
{
	...
	private static array $_colourSet = ['mediumpurple1', 'palegreen3', 'gold1', 'cadetblue1', 'darkmagenta', 'coral', 'dodgerblue3', 'eggplant', 'mediumblue', 'magenta', 'sandybrown', 'cyan', 'firebrick1', 'forestgreen', 'deeppink4', 'darkolivegreen', 'goldenrod2'];

	private static array $_markSet = ['diamond'	=> MARK_DIAMOND, 'square'	=> MARK_SQUARE, 'triangle'	=> MARK_UTRIANGLE, 'x'			=> MARK_X, 'star'		=> MARK_STAR, 'dot'		=> MARK_FILLEDCIRCLE, 'dash'		=> MARK_DTRIANGLE, 'circle'	=> MARK_CIRCLE, 'plus'		=> MARK_CROSS];

	private $_graph	= null;

	private static int $_plotColour	= 0;

	private static int $_plotMark	= 0;
	...
	private function _renderPlotScatter($groupID,$bubble) {
		...
		for($i = 0; $i < $seriesCount; ++$i) {
			...
			if ($scatterStyle == 'lineMarker') {
				...
				[$splineDataY, $splineDataX] = $spline->Get(count($dataValuesX) * self::$_width / 20);
	...
	public function render($outputDestination) {
		...
		switch ($chartType) {
			...
			case 'surfaceChart' :
				...
				break;
			case 'stockChart' :
				$this->_renderStockChart($groupCount);
				break;
			default	:
	 ...
	 */
	public function __construct(private readonly PHPExcel_Chart $_chart)
	{
		$this->_graph	= null;
