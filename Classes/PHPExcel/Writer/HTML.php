...
class PHPExcel_Writer_HTML extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
  ...
  */
 private bool $_embedImages	= FALSE;

	/**
  * Use inline CSS?
  */
 private bool $_useInlineCss = false;
  ...
  */
 private bool $_spansAreCalculated	= false;
  ...
  */
 private bool $_generateSheetNavigationBlock = true;
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	private function _calculateSpans()
	{
		...
		$sheetIndexes = $this->_sheetIndex !== null ?
			[$this->_sheetIndex] : range(0, $this->_phpExcel->getSheetCount() - 1);
				...
				while($r++ < $lr) {
					...
					while($c++ < $lc) {
						if ( !($c == $fc && $r == $fr) ) {
							...
							$this->_isSpannedCell[$sheetIndex][$r][$c] = ['baseCell' => [$fr, $fc]];
						} else {
