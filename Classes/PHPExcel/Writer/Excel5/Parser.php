...
class PHPExcel_Writer_Excel5_Parser
{
	...
	// +-% '^&<>=,;#()"{}
	final public const REGEX_SHEET_TITLE_UNQUOTED = '[^\*\:\/\\\\\?\[\]\+\-\% \\\'\^\&\<\>\=\,\;\#\(\)\"\{\}]+';
	...
	final public const REGEX_SHEET_TITLE_QUOTED = '(([^\*\:\/\\\\\?\[\]\\\'])+|(\\\'\\\')+)+';
	 ...
	 */
	function _convert(mixed $token)
	{
		if (preg_match("/\"([^\"]|\"\"){0,255}\"/", (string) $token)) {
		...
		} elseif (preg_match('/^\$?([A-Ia-i]?[A-Za-z])\$?(\d+)$/',(string) $token)) {
			return $this->_convertRef2d($token);

		// match external references like Sheet1!A1 or Sheet1:Sheet2!A1 or Sheet1!$A$1 or Sheet1:Sheet2!$A$1
		} elseif (preg_match("/^" . self::REGEX_SHEET_TITLE_UNQUOTED . "(\:" . self::REGEX_SHEET_TITLE_UNQUOTED . ")?\!\\$?[A-Ia-i]?[A-Za-z]\\$?(\d+)$/u",(string) $token)) {
			return $this->_convertRef3d($token);

		// match external references like 'Sheet1'!A1 or 'Sheet1:Sheet2'!A1 or 'Sheet1'!$A$1 or 'Sheet1:Sheet2'!$A$1
		} elseif (preg_match("/^'" . self::REGEX_SHEET_TITLE_QUOTED . "(\:" . self::REGEX_SHEET_TITLE_QUOTED . ")?'\!\\$?[A-Ia-i]?[A-Za-z]\\$?(\d+)$/u",(string) $token)) {
			return $this->_convertRef3d($token);

		// match ranges like A1:B2 or $A$1:$B$2
		} elseif (preg_match('/^(\$)?[A-Ia-i]?[A-Za-z](\$)?(\d+)\:(\$)?[A-Ia-i]?[A-Za-z](\$)?(\d+)$/', (string) $token)) {
			return $this->_convertRange2d($token);

		// match external ranges like Sheet1!A1:B2 or Sheet1:Sheet2!A1:B2 or Sheet1!$A$1:$B$2 or Sheet1:Sheet2!$A$1:$B$2
		} elseif (preg_match("/^" . self::REGEX_SHEET_TITLE_UNQUOTED . "(\:" . self::REGEX_SHEET_TITLE_UNQUOTED . ")?\!\\$?([A-Ia-i]?[A-Za-z])?\\$?(\d+)\:\\$?([A-Ia-i]?[A-Za-z])?\\$?(\d+)$/u",(string) $token)) {
			return $this->_convertRange3d($token);

		// match external ranges like 'Sheet1'!A1:B2 or 'Sheet1:Sheet2'!A1:B2 or 'Sheet1'!$A$1:$B$2 or 'Sheet1:Sheet2'!$A$1:$B$2
		} elseif (preg_match("/^'" . self::REGEX_SHEET_TITLE_QUOTED . "(\:" . self::REGEX_SHEET_TITLE_QUOTED . ")?'\!\\$?([A-Ia-i]?[A-Za-z])?\\$?(\d+)\:\\$?([A-Ia-i]?[A-Za-z])?\\$?(\d+)$/u",(string) $token)) {
			return $this->_convertRange3d($token);
        ...
        // match error codes
		} elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", (string) $token) or $token == '#N/A') {
	 ...
	 */
	function _convertNumber(mixed $num)
	{
		// Integer in the range 0..2**16-1
		if ((preg_match("/^\d+$/", (string) $num)) and ($num <= 65535)) {
...
//		$volatile = $this->_functions[$token][3];
	...
	function _convertRange2d($range, $class=0)
	{
		...
		[$row1, $col1] = $this->_cellToPackedRowcol($cell1);
		[$row2, $col2] = $this->_cellToPackedRowcol($cell2);
...
//		$class = 0; // formulas like Sheet1!$A$1:$A$2 in list type data validation need this class (0x3B)
		...
		if (preg_match("/^(\\$)?[A-Ia-i]?[A-Za-z](\\$)?(\d+)$/", $cell1)) {
			...
			[$row2, $col2] = $this->_cellToPackedRowcol($cell2);
			 ...
			 [$row1, $col1, $row2, $col2] = $this->_rangeToPackedRange($cell1.':'.$cell2);
		}
...
//		$class = 2; // as far as I know, this is magick.
		...
		[$row, $col] = $this->_cellToPackedRowcol($cell);
...
//		}
	...
	function _cellToPackedRowcol($cell)
	{
		...
		[$row, $col, $row_rel, $col_rel] = $this->_cellToRowcol($cell);
		if ($col >= 256) {
	...
	function _match(mixed $token)
	{
		switch($token) {
			...
			default:
				...
				if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/',(string) $token) and
				...
				elseif (preg_match("/^" . self::REGEX_SHEET_TITLE_UNQUOTED . "(\:" . self::REGEX_SHEET_TITLE_UNQUOTED . ")?\!\\$?[A-Ia-i]?[A-Za-z]\\$?[0-9]+$/u",(string) $token) and
					   !preg_match("/[0-9]/",$this->_lookahead) and
				...
				// If it's an external reference ('Sheet1'!A1 or 'Sheet1:Sheet2'!A1 or 'Sheet1'!$A$1 or 'Sheet1:Sheet2'!$A$1)
				elseif (preg_match("/^'" . self::REGEX_SHEET_TITLE_QUOTED . "(\:" . self::REGEX_SHEET_TITLE_QUOTED . ")?'\!\\$?[A-Ia-i]?[A-Za-z]\\$?[0-9]+$/u",(string) $token) and
					   !preg_match("/[0-9]/",$this->_lookahead) and
					   ($this->_lookahead != ':') and ($this->_lookahead != '.'))
				{
					return $token;
				...
				elseif (preg_match('/^(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+:(\$)?[A-Ia-i]?[A-Za-z](\$)?[0-9]+$/', (string) $token) and
					   !preg_match("/[0-9]/",$this->_lookahead))
				{
					return $token;
				}
				// If it's an external range like Sheet1!A1:B2 or Sheet1:Sheet2!A1:B2 or Sheet1!$A$1:$B$2 or Sheet1:Sheet2!$A$1:$B$2
				elseif (preg_match("/^" . self::REGEX_SHEET_TITLE_UNQUOTED . "(\:" . self::REGEX_SHEET_TITLE_UNQUOTED . ")?\!\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+:\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+$/u",(string) $token) and
					   !preg_match("/[0-9]/",$this->_lookahead))
				{
					return $token;
				}
				// If it's an external range like 'Sheet1'!A1:B2 or 'Sheet1:Sheet2'!A1:B2 or 'Sheet1'!$A$1:$B$2 or 'Sheet1:Sheet2'!$A$1:$B$2
				elseif (preg_match("/^'" . self::REGEX_SHEET_TITLE_QUOTED . "(\:" . self::REGEX_SHEET_TITLE_QUOTED . ")?'\!\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+:\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+$/u",(string) $token) and
					   !preg_match("/[0-9]/",$this->_lookahead))
				{
					return $token;
				}
				...
				elseif (preg_match("/\"([^\"]|\"\"){0,255}\"/",(string) $token) and $this->_lookahead != '"' and (substr_count((string) $token, '"')%2 == 0))
				{
					return $token;
				}
				...
				elseif (preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/i",(string) $token) and ($this->_lookahead == "("))
				{
					return $token;
				}
				//	It's an argument of some description (e.g. a named range),
				//		precise nature yet to be determined
				elseif(str_ends_with((string) $token, ')')) {
					return $token;
				}
				return '';
		}
	 ...
	 * @return array A tree
	 */
	function _createTree(mixed $value, mixed $left, mixed $right)
	{
	...
	function toReversePolish($tree = [])
	{
		...
		if (preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/",(string) $tree['value']) and
			!preg_match('/^([A-Ia-i]?[A-Za-z])(\d+)$/',(string) $tree['value']) and
			!preg_match("/^[A-Ia-i]?[A-Za-z](\d+)\.\.[A-Ia-i]?[A-Za-z](\d+)$/",(string) $tree['value']) and
