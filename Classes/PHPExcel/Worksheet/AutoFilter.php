...
class PHPExcel_Worksheet_AutoFilter implements \Stringable
{
     ...
     *
     * @param string $_range Cell range (i.e. A1:E10)
     * @param PHPExcel_Worksheet $_workSheet
     */
 ...
 public function setParent(PHPExcel_Worksheet $pSheet = NULL) {
	...
	public function setRange($pRange = '') {
		...
		if (empty($pRange)) {
			...
			[$rangeStart, $rangeEnd] = PHPExcel_Cell::rangeBoundaries($this->_range);
  ...
  *	@return boolean
  */
 private static function _filterTestInSimpleDataSet(mixed $cellValue,$dataSet)
	{
  ...
  *	@return boolean
  */
 private static function _filterTestInDateGroupSet(mixed $cellValue,$dataSet)
	{
  ...
  *	@return boolean
  */
 private static function _filterTestInCustomDataSet(mixed $cellValue, $ruleSet)
	{
  ...
  *	@return boolean
  */
 private static function _filterTestInPeriodDateSet(mixed $cellValue, $monthSet)
	{
		//	Blank cells are always ignored, so return a FALSE
		if (($cellValue == '') || ($cellValue === NULL)) {
			return FALSE;
	...
	private function _dynamicFilterDateRange($dynamicRuleType, &$filterColumn)
	{
		...
		$ruleValues[] = ['operator' => PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_GREATERTHANOREQUAL, 'value' => $val];
		$ruleValues[] = ['operator' => PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_COLUMN_RULE_LESSTHAN, 'value' => $maxVal];
		PHPExcel_Calculation_Functions::setReturnDateType($rDateType);

		return ['method' => '_filterTestInCustomDataSet', 'arguments' => ['filterRules' => $ruleValues, 'join' => PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_AND]];
	}
			...
			switch ($filterColumn->getFilterType()) {
				case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER :
					...
					if ($ruleType == PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_FILTER) {
						...
						$columnFilterTests[$columnID] = ['method' => '_filterTestInSimpleDataSet', 'arguments' => ['filterValues' => $ruleDataSet, 'blanks' => $blanks]];
					} else {
						...
						$columnFilterTests[$columnID] = ['method' => '_filterTestInDateGroupSet', 'arguments' => ['filterValues' => $arguments, 'blanks' => $blanks]];
					}
					break;
				case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_CUSTOMFILTER :
					...
					$columnFilterTests[$columnID] = ['method' => '_filterTestInCustomDataSet', 'arguments' => ['filterRules' => $ruleValues, 'join' => $join, 'customRuleForBlanks' => $customRuleForBlanks]];
					break;
				case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER :
					...
					foreach($rules as $rule) {
						...
						if (($dynamicRuleType == PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_ABOVEAVERAGE) ||
							...
							$ruleValues[] = ['operator' => $operator, 'value' => $average];
							$columnFilterTests[$columnID] = ['method' => '_filterTestInCustomDataSet', 'arguments' => ['filterRules' => $ruleValues, 'join' => PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_OR]];
						} else {
							//	Date based
							if ($dynamicRuleType[0] == 'M' || $dynamicRuleType[0] == 'Q') {
								...
								$columnFilterTests[$columnID] = ['method' => '_filterTestInPeriodDateSet', 'arguments' => $ruleValues];
...
//				echo 'Value is ',$cellValue,PHP_EOL;
				...
				$result = $result &&
					call_user_func_array(
						['PHPExcel_Worksheet_AutoFilter', $columnFilterTest['method']],
